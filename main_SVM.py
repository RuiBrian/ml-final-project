...
def fit_predict(flanking_seq):
    ...
    X_dev = pd.read_csv(
        f"datasets/processed/{flanking_seq}nt_dev_separated.csv",
        header=None,
        index_col=False,
    )
    ...
    np.savetxt(
        f"predictions/{flanking_seq}nt_SVM_dev_predictions.csv",
        y_pred,
        fmt="%i",
    )
    np.savetxt(
        f"predictions/{flanking_seq}nt_SVM_dev_softpredictions.csv",
        y_softpred,
        fmt="%f",
    )

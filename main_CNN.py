...
def train(device, flanking_seq):
    ...
    TRAIN_SEQUENCES = np.load(f"datasets/processed/{flanking_seq}nt_train_encoded.npy")
    TRAIN_LABELS = np.load(f"datasets/processed/{flanking_seq}nt_train_labels.npy")
...
def predict(device, flanking_seq, dev_or_test):
    ...
    for i in range(0, DEV_SEQUENCES.shape[0], HEIGHT):
        ...
        y = torch.from_numpy(DEV_LABELS[cur_label_index].astype(int)).to(device)
    ...
    print(f"Accuracy: {metrics.accuracy_score(DEV_LABELS.ravel(), predictions)}")
...
if __name__ == "__main__":
    ...
    if MODE == "train":
        ...
        LOGFILETRAIN = open(os.path.join(f"logs/{flanking_seq}nt_train_CNN.log"), "w")
        ...
        LOGFILEDEV = open(os.path.join(f"logs/{flanking_seq}nt_dev_CNN.log"), "w")
        ...
        LOGFILETEST = open(os.path.join(f"logs/{flanking_seq}nt_test_CNN.log"), "w")
